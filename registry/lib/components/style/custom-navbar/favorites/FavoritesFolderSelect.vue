...
import { type FavoritesFolder, notSelectedFolder } from './favorites-folder'
...
const emit = defineEmits<{ change: [value: FavoritesFolder] }>()

const { folder } = defineProps<{ folder: FavoritesFolder }>()

const change = (selectedFolder: FavoritesFolder) => {
  navbarOptions.lastFavoriteFolder = selectedFolder.id
  emit('change', selectedFolder)
}
