  ...
  <div class="user-info-panel">
    <div v-if="isLogin && userInfo.isLogin === true" class="logged-in">
      ...
      <div class="level-progress separator">
        <div class="level-progress-thumb" :style="levelProgressStyle" />
      </div>
      <div class="items">
        <a
          class="item"
        ...
        >
          <VIcon icon="bind-phone" :size="20" />
          <VIcon v-if="userInfo.mobile_verified" icon="ok" :size="18" />
          <VIcon v-else icon="cancel" :size="18" />
        </a>
        <a
          class="item"
          target="_blank"
...
import { usePopper, type UsePopperProps } from '../mixins'

const { VIcon } = coreApis.ui
...
const privileges = ref<Privileges>({
  bCoin: {
    received: false,
    expire: '',
  },
...
//   if (!userInfo.value.level_info) return { icon: 'lv0' }
//
//   const baseLevel = `lv${userInfo.value.level_info.current_level}`
//   if (userInfo.value.is_senior_member) {
//     return {
//       icon: `${baseLevel}-plus`,
//       colored: true,
//     }
...
const userType = computed(() => {
  ...
  if (userInfo.value.level_info?.current_level === 0) {
    return '注册会员'
  }
  if (userInfo.value.vipStatus === 1) {
    if (userInfo.value.vipType === 1) {
      return userInfo.value.vip_theme_type ? '小会员' : '大会员'
    }
    if (userInfo.value.vipType === 2) {
      return userInfo.value.vip_theme_type ? '年度小会员' : '年度大会员'
    }
  }
  return '正式会员'
})
...
const levelProgressStyle = computed(() => {
  if (!userInfo.value.isLogin || !userInfo.value.level_info) {
    return {}
  }
  const { current_exp, current_min, next_exp } = userInfo.value.level_info
...
const privilegeReceive = async (type: PrivilegeType) => {
  const typeMapping = {
    1: 'bCoin',
    2: 'coupons',
  } as const

  const privilegeKey = typeMapping[type]
  if (privileges.value[privilegeKey].received) {
    return
  }

  privileges.value[privilegeKey].received = true
  const csrf = getCsrf()
  ...
  try {
    ...
    console.log(result)
    if (result.code === 0) {
      if (privilegeKey === 'bCoin' && userInfo.value.wallet) {
        userInfo.value.wallet.bcoin_balance += 5
      }
    } else if (result.code === 69801) {
      // 已领过
    } else {
      privileges.value[privilegeKey].received = false
...
const init = async () => {
  ...
  stat.value = json.data || {}

  if (isLogin.value && userInfo.value.vipType === 2) {
    // 年度大会员权益
    const privilegesData = await getJsonWithCredentials(
      'https://api.bilibili.com/x/vip/privilege/my',
    )
    if (privilegesData.code === 0) {
      const bCoin = privilegesData.data.list.find((it: { type: PrivilegeType }) => it.type === 1)
      if (bCoin) {
        privileges.value.bCoin.received = bCoin.state === 1
        privileges.value.bCoin.expire = new Date(bCoin.expire_time * 1000).toLocaleDateString()
      }

      const coupons = privilegesData.data.list.find((it: { type: PrivilegeType }) => it.type === 2)
      if (coupons) {
        privileges.value.coupons.received = coupons.state === 1
        privileges.value.coupons.expire = new Date(coupons.expire_time * 1000).toLocaleDateString()
      }
    }
  }
}
