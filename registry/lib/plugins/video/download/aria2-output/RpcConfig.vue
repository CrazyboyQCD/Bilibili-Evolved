  ...
  <div class="rpc-config download-video-config-section">
    <div class="online-assets-download">
      <div class="download-video-config-item">
        ...
        <SwitchBox :checked="isPluginDownloadAssets" @change="saveAssetsSettings($event)" />
      </div>
    ...
    <div v-if="isRenaming" class="profile-select">
      ...
      <TextBox ref="renameInput" :text="profileRename" @change="profileRename = $event" />
    ...
    <div v-else class="profile-select">
      ...
      <VDropdown
        ...
        :items="rpcProfiles"
        :key-mapper="item => item.name"
        @change="selectedRpcProfile = $event"
      >
        <template #item="{ item }">
          {{ item.name }}
        </template>
    ...
    <template v-if="selectedRpcProfile">
      <div class="profile-secret-key">
        ...
        <TextBox
          :text="selectedRpcProfile.secretKey"
          change-on-blur
          @change="selectedRpcProfile.secretKey = $event"
        />
      </div>
      <div class="profile-dir">
        <div class="profile-item-name">路径:</div>
        <TextBox
          :text="selectedRpcProfile.dir"
          change-on-blur
          @change="selectedRpcProfile.dir = $event"
        />
      </div>
      <div class="profile-host">
        <div class="profile-item-name">主机:</div>
        <TextBox
          :text="selectedRpcProfile.host"
          change-on-blur
          @change="selectedRpcProfile.host = $event"
        />
      </div>
      <div class="profile-port">
        <div class="profile-item-name">端口:</div>
        <TextBox
          :text="selectedRpcProfile.port"
          change-on-blur
          @change="selectedRpcProfile.port = $event"
        />
      </div>
      <div class="profile-method">
        <div class="profile-item-name">方法:</div>
        <VDropdown
          :value="selectedRpcProfile.method"
          :items="['get', 'post']"
          :key-mapper="item => item"
          @change="selectedRpcProfile.method = $event"
        >
          <template #item="{ item }">
            {{ item }}
          </template>
      ...
      <div class="profile-other">
        ...
        <TextArea
          :text="selectedRpcProfile.other"
          placeholder="some-key=value"
          change-on-blur
          @change="selectedRpcProfile.other = $event"
        />
      </div>
    </template>
  </div>
...
import { type Aria2RpcProfile, defaultProfile } from './rpc-profiles'
...
const saveAssetsSettings = (checked: boolean) => {
  isPluginDownloadAssets.value = checked
  options.isPluginDownloadAssets = checked
  storedOptions.isPluginDownloadAssets = checked
}
