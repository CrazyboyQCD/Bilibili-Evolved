...
const injectCoreInfo: InjectMetadataAction = ({ code }) => {
  ...
  if (componentMatch) {
    ...
    return code.slice(0, insertIndex) + coreInfoProperties + code.slice(insertIndex);
  }
  return code;
};
...
const injectDescription: InjectMetadataAction = ({ code, filename }) => {
  ...
  const matchedFiles = readdirSync(folder).filter((file) => descRegex.test(file));

  // 检查是否有对应的语言描述文件
  let descCode = "";
  if (matchedFiles.length > 0) {
    // 添加所有匹配的语言文件
    const langImports = matchedFiles
      .map((file) => {
  ...
  if (componentMatch) {
    const insertIndex = componentMatch.index! + componentMatch[0].length;
    return code.slice(0, insertIndex) + descCode + code.slice(insertIndex);
  }
  return code;
};
...
const injectI18n: InjectMetadataAction = ({ code, filename }) => {
  ...
  if (componentMatch) {
    const insertIndex = componentMatch.index! + componentMatch[0].length;
    return code.slice(0, insertIndex) + i18nCode + code.slice(insertIndex);
  }
  return code;
};
