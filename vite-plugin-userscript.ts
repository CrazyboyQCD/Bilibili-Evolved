  ...
  return {
    ...
    transform(code) {
      ...
      transformedCode = transformedCode.replace(
        /\bwebpackCompilationInfo\b/g,
        JSON.stringify(compilationInfo),
      );
      ...
      for (const [fileName, bundleItem] of Object.entries(bundle)) {
        if (bundleItem.type === "chunk" && bundleItem.isEntry) {
          const commonMeta = JSON.parse(
            fs.readFileSync(path.resolve(__dirname, "src/client/common.meta.json"), "utf-8"),
          );
          ...
          for (const [key, value] of Object.entries(fullMeta)) {
            if (Array.isArray(value)) {
              const lines = [...new Set(value.map((item) => `// @${key.padEnd(16, " ")}${item}`))];
              banner += lines.join("\n") + "\n";
            } else {
